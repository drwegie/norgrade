/**
 * Lens 1 (home): the socioeconomic gradient in lower secondary results, by
 * parents' education, from SSB table 11689.
 *
 * The page itself is a server component and holds no data: the snapshot is
 * imported by `GradeLens`, which owns the selected cross-section as
 * `useState`. The route is still prerendered to static HTML -- no lens
 * reads `useSearchParams`, so nothing is deferred to the browser and
 * `next build` reports this route as `○ (Static)`. Per
 * docs/adr/ADR-001-ssb-ingest-boundary.md the number of SSB requests made
 * while serving it is zero.
 */

import type { Metadata } from "next";
import { GradeLens } from "./grade-lens";
